...
class TableroProc():
    ...
    def dataFrame_from_file(self):
        if self.checkInFTP():
            df = pd.read_excel(
                self.path_arch, self.hc, index_col=None,
                engine='openpyxl', header=self.hdr)
    ...
    def process(self):
        ...
        with connection.cursor() as cursor:
            for query in [
                    f"""DELETE FROM app_tablero_estadistico
                        WHERE cuenta_id IN (
                            SELECT id
                            FROM app_tablero_cuenta
                            WHERE tablero_id = {self.tabl.pk});""",
                    f"""DELETE FROM app_tablero_estadisticoanual
                        WHERE cuenta_id IN (
                            SELECT id
                            FROM app_tablero_cuenta
                            WHERE tablero_id = {self.tabl.pk});"""]:
        ...
        df['cuenta_id'] = df.apply(
            self.process_fila, axis=1, periodos=periodos)
        for per in periodos:
